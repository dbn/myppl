define([
  ...
  'backbone',
  'model/cities',
  'view/home/homecityview',
  'view/home/homecitydetailedview',
  'view/home/homeartistdetailedview',
  'text!templates/home/hometmpl.html'
],

function(   $, _, Backbone, CollectionOfCities , CityView , 
            CityDetailedView , ArtistDetailedView , homeTemplate)
{
    ...
    initialize: function()
    {
      this.collection = CollectionOfCities;
    ...
    collectionChangeHandler: function(){
        ...
        {
            var cityView = new CityView({model:cities[i]});
    ...
    {           
        ...
        Backbone.history.navigate("/city/" + cityName.toLowerCase(),{trigger: true});
    ...
    showCity:function(cityName){
        ...
        {   
            if (!sameCity)
            {
                $.when(this.showHome()).done(function(){thisRef.showCity(cityName)});            
            }
            else 
            {
               $.when(this.hidePrevDetailedView()).done(
                    function(){thisRef.showCityDetails(cityView)});
            }
        }
        else
        {   
    ...
    {    
        ...
        Backbone.history.navigate("/artist/" + artistName.toLowerCase(),{trigger: true});
    },
    
    showArtist:function(name){
        
        var artistModel = CollectionOfCities.findArtist(name);
        if (!artistModel) return;
        var cityView = this.getCityViewByName(artistModel.get("city")); 
        if(!cityView) return;
        
        
        cityView.model.set({selectedArtist:artistModel});
        artistModel.set({"selected":true});
        ...
        {  
            ...
            {
                $.when(this.showHome()).done(function(){thisRef.showArtist(name)});
            }
            else
            {
                $.when(this.hidePrevDetailedView()).done(
                    function(){thisRef.showArtistDetails(cityView)});
            }
        }
        else if (this.currentView == "city")
        {            
            $.when(this.hidePrevDetailedView()).done(function(){thisRef.showArtistDetails(cityView)});
        }
        else
        {   
    ...
    elevateCityView:function(cityView){
        ...
        var hiddenCityViewsElArr = _.map(this.hiddenCityViews,function(view){return view.el;});
        ...
        $.when(allFadeOurEffect()).done(function(){
            ...
            if (viewIndex != 0)
            {
                $(cityView.el).animate({top:0},500,function(){
    ...
    showArtistDetails : function(cityView){
        ...
        $.when(detEl.fadeIn(200)).done(function(){cityView.showBackButton()});
    ...
    hidePrevDetailedView:function(){
        ...
        $.when($(detView.el).fadeOut(500)).done(
            function(){        
                $(detView.el).detach();    
                dfrd.resolve();
                }
